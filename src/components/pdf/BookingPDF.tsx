...
export default function BookingPDF({ request }: BookingPDFProps) {
  const isSelected = (optLabel: string) => {
    ...
    const isTaxi = request.pickupMethod === "TAXI";
    const isSelfDrive = request.selfDriveBool === true;

    if (optLabel === "อนุมัติ(TAXI)") return isTaxi && isApproved;
    if (optLabel === "อนุมัติ(ผู้ขอขับเอง)")
      return isSelfDrive && !isTaxi && isApproved;
    if (optLabel === "อนุมัติ") return isApproved && !isSelfDrive && !isTaxi;
    if (optLabel === "ไม่อนุมัติ") return isRejected;
    return false;
  };
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <SectionBox title="ส่วนการขอใช้งานรถยนต์">
          <PDFRow
            label="ผู้ขอใช้"
            value={`${request.requesterUsername ?? request.id} - ${request.requester}`}
          />
          <PDFRow label="สังกัดผู้ขอใช้งาน" value={request.department} />
          <PDFRow label="หมายเลขโทรศัพท์" value={request.phone} />
          <PDFRow
            label="วันเดินทางไป"
            value={`${request.date} เวลา ${request.time} น.`}
          />
          <PDFRow
            label="วันเดินทางกลับ"
            value={`${request.endDate} เวลา ${request.endTime} น.`}
          />
          ...
          <PDFRow
            ...
            value={
              request.approverUsername
                ? `${request.approverUsername} - ${request.approver}`
                : request.approver || "-"
            }
          />
        </SectionBox>
        ...
        <SectionBox title="ส่วนการจัดการข้อมูลการขอใช้งานรถยนต์">
          ...
          <PDFRow
            label="สถานการณ์ขอใช้งาน"
            value={getStatusName(request.status)}
          />
          <PDFRow
            label="ผู้ดำเนินการ "
            value={
              request.dispatcherUsername
                ? `${request.dispatcherUsername} - ${request.dispatcher}`
                : request.dispatcher || "-"
            }
          />
        </SectionBox>
