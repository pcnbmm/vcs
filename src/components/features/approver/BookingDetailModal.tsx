...
function StatusBadge({
  ...
  if (isExpired) {
    return (
      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider bg-orange-100 text-orange-700">
        เกินกำหนด
      </span>
    );
  }
  ...
  return (
    <span
      className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider ${className}`}
    >
      {label}
    </span>
  );
        ...
        <div className="p-5">
          ...
          <div className="space-y-5">
            ...
            <Section title="เส้นทางที่ต้องการเดินทาง" icon={MapPin}>
              <div className="flex items-center gap-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <span className="bg-white px-2.5 py-1 rounded-lg text-xs font-bold text-slate-600 shadow-sm border border-slate-100 truncate max-w-[150px]">
                  {booking.origin}
                </span>
                <ArrowRight size={14} className="text-slate-300 shrink-0" />
                <span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-xs font-bold shadow-lg shadow-blue-100 truncate">
                  {booking.destination}
                </span>
              </div>
            </Section>
              ...
              <Section title="วันเวลาเดินทาง" icon={Calendar}>
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-slate-700">
                    กลับ:{" "}
                    <span className="text-slate-900 font-black">
                      {formatThaiDateTime(booking.endDateTime)}
                    </span>
                  </p>
                </div>
              </Section>
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-tight">
                    <span className="text-slate-400">
                      ผู้โดยสาร {booking.passengerCount} คน
                    </span>
                    <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md italic">
                      {booking.selfDrive}
                    </span>
                  </div>
                </div>
          ...
          {booking.status === "PENDING" && (
            <div className="mt-6 pt-6 border-t border-slate-100">
              {/* ✅ ถ้าเกินกำหนด → แสดง notice ล็อกปุ่มไว้ */}
              {isExpired ? (
                <div className="flex items-center gap-3 justify-center bg-orange-50 border border-orange-100 rounded-xl p-4">
                  <span className="text-sm font-bold text-orange-600">
                    ⏰ เลยกำหนดเวลาการใช้งานแล้ว ไม่สามารถอนุมัติหรือปฏิเสธได้
                  </span>
                </div>
              ) : isRejecting ? (
