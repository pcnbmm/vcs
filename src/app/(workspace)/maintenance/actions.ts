  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const maintenanceData: any = {
        ...
        mile_car_in: data.mile_car_in !== undefined && data.mile_car_in !== null ? Number(data.mile_car_in) : null,
        mile_car_out: data.mile_car_out !== undefined && data.mile_car_out !== null ? Number(data.mile_car_out) : null,
        finish_date: data.finish_date ? new Date(data.finish_date) : null,
        finish_time: data.finish_time,
        vat: data.vat !== undefined && data.vat !== null ? Number(data.vat) : null,
      ...
      if (data.spare_items && data.spare_items.length > 0) {
        await tx.vc_maintenance_spare_item.createMany({
          data: data.spare_items.map((item) => ({
            ...
            spare_amount: item.amount !== undefined && item.amount !== null ? Number(item.amount) : null,
            spare_price: item.price !== undefined && item.price !== null ? Number(item.price) : null,
            cre_by: data.cre_by,
            cre_date: new Date(),
            upd_by: data.cre_by,
            upd_date: new Date(),
