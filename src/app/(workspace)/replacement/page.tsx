...
import { Plus, X, Loader2, RotateCcw, CheckCircle, Clock, Eye, Edit2, Search } from "lucide-react";
...
export default function ReplacementPage() {
    ...
    const openModal = (mode: 'add' | 'view' | 'edit', record?: any) => {
        ...
        if (mode === 'add') {
            setSelectedId(null);
            setFormData({ 
                car_id: "", replacement_car_number: "", car_province_id: "", car_spec_id: "", remark: "",
                broken_car_id: "", start_date: getLocalISOString(new Date()), broken_datetime: getLocalISOString(new Date()), end_date: getLocalISOString(new Date()), end_datetime: "", cre_by: ""
            });
            fetchAvailableCars();
        } else if (record) {
            ...
            setFormData({
                ...
                start_date: getLocalISOString(record.start_datetime || record.start_date || new Date()),
                broken_datetime: getLocalISOString(record.broken_datetime || new Date()),
                end_date: getLocalISOString(record.end_datetime || record.end_date || new Date()),
    ...
    const handleCheckCar = async () => {
        ...
        setIsChecking(true);
        try {
            const res = await fetch(`/api/replacements/check?plate=${encodeURIComponent(formData.replacement_car_number)}&province=${formData.car_province_id}`);
            if (res.ok) {
                const data = await res.json();
                if (data.exists) {
                    showWarning("รถคันนี้มีในระบบแล้ว ให้นำทะเบียนรถไปค้นหาที่หน้าแรกเพื่อดำเนินการจัดการต่อไป");
                    closeModal();
                } else {
                    setIsChecked(true);
                    showSuccess("ไม่พบข้อมูลในระบบ กรุณากรอกประเภทรถเพื่อบันทึกเป็นรถทดแทนใหม่");
                }
            }
        } catch (error) {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-md shadow-sm border border-gray-100">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500">เพิ่มและจัดการข้อมูลการใช้งานรถทดแทน</p>
                </div>
                {hasAccess('create') && (
                    <button 
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {isLoading ? (
                                ...
                                replacements.map((r) => {
                                    ...
                                    return (
                                        <tr key={r.replacement_id} className={`hover:bg-slate-50 transition-colors ${isActive ? "bg-blue-50/20" : isIdle ? "bg-amber-50/20" : ""}`}>
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-white shrink-0">
                            <h2 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                                ...
                                {modalMode === 'add' ? 'เพิ่มข้อมูลรถทดแทน' : modalMode === 'edit' ? 'จัดการรถทดแทน' : 'รายละเอียดการใช้รถทดแทน'}
                            </h2>
                        </div>
                        ...
                        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
                            ...
                            {modalMode === 'add' && (
                                <>
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-semibold text-gray-800">ทะเบียนรถที่มาทดแทน <span className="text-rose-500">*</span></label>
                                        <input 
                                            type="text"
                                            required
                                            disabled={isChecked}
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-semibold text-gray-800">จังหวัดที่จดทะเบียน <span className="text-rose-500">*</span></label>
                                        <Select 
                                            options={provinces.map(p => ({ value: p.province_id.toString(), label: p.province_name }))}
                                            isDisabled={isChecked}
                                            placeholder="-- เลือกจังหวัด --"
                                            value={provinces.find(p => p.province_id.toString() === formData.car_province_id) ? {
                                                value: formData.car_province_id,
                                                label: provinces.find(p => p.province_id.toString() === formData.car_province_id)?.province_name
                                            } : null}
                                            onChange={(selected: any) => setFormData({...formData, car_province_id: selected ? selected.value : ""})}
                                    ...
                                    {!isChecked ? (
                                        ...
                                        >
                                            {isChecking ? <Loader2 className="w-5 h-5 animate-spin" /> : <Search className="w-4 h-4" />}
                                            ตรวจสอบทะเบียนรถ
                                        </button>
                                    ) : (
                                        <div className="space-y-1.5 animate-in fade-in slide-in-from-top-4 duration-300">
                                            <label className="text-sm font-semibold text-gray-800">ประเภทรถ <span className="text-rose-500">*</span></label>
                                            <Select 
                                                options={carTypes.map(c => ({ value: c.car_type_id.toString(), label: c.car_type_name }))}
                                                placeholder="-- เลือกประเภทรถ --"
                                                value={carTypes.find(c => c.car_type_id.toString() === formData.car_spec_id) ? {
                                                    value: formData.car_spec_id,
                                                    label: carTypes.find(c => c.car_type_id.toString() === formData.car_spec_id)?.car_type_name
                                                } : null}
                                                onChange={(selected: any) => setFormData({...formData, car_spec_id: selected ? selected.value : ""})}
                                                className="react-select-container text-sm font-medium"
                                                classNamePrefix="react-select"
                                            />
                                        </div>
                                ...
                                <>
                                    <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl space-y-2">
                                        <div className="flex justify-between items-center text-sm">
                                            <span className="font-semibold text-blue-900">ทะเบียนรถทดแทน:</span>
                                            <span className="font-bold text-blue-700 bg-white px-2 py-0.5 rounded border border-blue-200">{formData.replacement_car_number}</span>
                                        </div>
                                    </div>
                                    ...
                                    {!isReturning ? (
                                        <>
                                            <div className="space-y-1.5">
                                                ...
                                                {modalMode === 'edit' && !formData.broken_car_id && (
                                                    <p className="text-xs text-slate-500 mt-1">ระบบจะเปลี่ยนทะเบียนของรถคันนี้บนระบบเป็นคันใหม่ชั่วคราว</p>
                                                )}
                                            </div>

                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5">
                                                    <label className="text-sm font-semibold text-gray-800">วัน/เวลาที่แจ้งเหตุเสีย <span className="text-rose-500">*</span></label>
                                                    <input 
                                                        type="datetime-local" 
                                                        disabled={modalMode === 'view' || !!formData.broken_car_id}
                                                        value={formData.broken_datetime}
                                                        onChange={e => setFormData({...formData, broken_datetime: e.target.value})}
                                                ...
                                                <div className="space-y-1.5">
                                                    <label className="text-sm font-semibold text-gray-800">วันที่และเวลาเริ่มทดแทน <span className="text-rose-500">*</span></label>
                                                    <input 
                                                        type="datetime-local" 
                                                        disabled={modalMode === 'view' || !!formData.broken_car_id}
                                            ...
                                            {formData.end_datetime && (
                                                <div className="space-y-1.5 pt-2">
                                                    <label className="text-sm font-semibold text-gray-800">วันที่และเวลาที่ส่งคืนรถ <span className="text-emerald-500 font-bold">(ส่งคืนแล้ว)</span></label>
                                                    <input 
                                                        type="datetime-local" 
                                                        disabled
                                        ...
                                        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                                            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                                <p className="text-xs text-amber-800 leading-relaxed font-medium">
                                                    ระบบจะคืนค่าทะเบียนรถและสถานะของรถเดิม (<span className="font-bold">{formData.broken_car_id}</span>) กลับสู่สภาวะปกติ และนำข้อมูลวันเวลาไปใช้สำหรับรายงานการใช้รถ-รายเดือน
                                                </p>
                                            </div>
                                            <div className="space-y-1.5">
                                                <label className="text-sm font-semibold text-gray-800">วันที่และเวลาที่ส่งคืนรถ <span className="text-rose-500">*</span></label>
                                                <input 
                                                    type="datetime-local" 
                                                    required
                                                    value={formData.end_date}
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
                                ...
                                {modalMode !== 'view' && (modalMode !== 'add' || isChecked) && (
                                    <button type="submit" disabled={isSaving} className={`flex items-center gap-2 px-6 py-2.5 text-white rounded-lg font-bold text-sm shadow-md transition-all disabled:opacity-50 ${isReturning ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-200' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200'}`}>
                                        {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
                                        {isSaving ? "กำลังบันทึก..." : (isReturning ? "ยืนยันส่งคืนรถทดแทน" : "บันทึกข้อมูล")}
                                    </button>
                                )}
                            </div>
                        </form>
