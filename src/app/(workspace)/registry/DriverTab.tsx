...
export default function DriverTab() {
    ...
    const filteredDrivers = drivers.filter(d => {
        const fullStr = `${d.driver_code} ${d.vc_users?.firstname} ${d.vc_users?.lastname} ${d.tel} ${d.licence_no}`.toLowerCase();
    ...
    const openModal = (mode: 'view' | 'add' | 'edit', driver?: any) => {
        ...
        if (mode === 'add') {
            setFormData({
                driver_code: '', driver_status: 'A', div_code: '', 
                start_date: '', end_date: '', licence_type: '', 
                licence_no: '', licence_by: '', tel: '', flag: 'Y'
            });
        } else {
            setFormData({
                ...driver,
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            const url = modalMode === 'add' ? '/api/drivers' : `/api/drivers/${selectedDriver.driver_id}`;
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={5} className="py-12 text-center">
                                        ...
                                        <p className="mt-4 text-sm font-medium text-gray-500">กำลังโหลดข้อมูลคนขับ...</p>
                                    </td>
                                </tr>
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span className="block text-xs text-gray-500">{driver.licence_no || 'ยังไม่ระบุใบขับขี่'}</span>
                                                </div>
                                            </div>
                                        </td>
                ...
                {!isLoading && totalPages > 1 && (
                    <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
                        <span className="text-sm font-medium text-gray-500">
                            แสดง {((currentPage - 1) * itemsPerPage) + 1} ถึง {Math.min(currentPage * itemsPerPage, filteredDrivers.length)} จาก {filteredDrivers.length} รายการ
                        </span>
                        ...
                        <div className="p-8 overflow-y-auto custom-scrollbar flex-1 bg-gray-50/50">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                <FormSection title="ข้อมูลส่วนตัว">
                                    <SelectField label="รหัสผู้ใช้งาน (User)" value={formData.driver_code} onChange={(v:any) => setFormData({...formData, driver_code: v})} options={options?.users} valueKey="userid" labelKey="firstname" labelKey2="lastname" disabled={modalMode === 'view'} />
                                    <InputField label="เบอร์โทรศัพท์" value={formData.tel} onChange={(v:any) => setFormData({...formData, tel: v})} disabled={modalMode === 'view'} />
                                    <SelectField label="สถานะการทำงาน" value={formData.driver_status} onChange={(v:any) => setFormData({...formData, driver_status: v})} options={[{id:'A', name:'พร้อมใช้งาน (A)'}, {id:'I', name:'ไม่พร้อมใช้งาน (I)'}]} valueKey="id" labelKey="name" disabled={modalMode === 'view'} />
                                </FormSection>

                                <FormSection title="ข้อมูลสังกัด">
                                    <SelectField label="รหัสหน่วยงาน (Div Code)" value={formData.div_code} onChange={(v:any) => setFormData({...formData, div_code: v})} options={options?.orgs} valueKey="orgid" labelKey="orgname" disabled={modalMode === 'view'} />
                                    <InputField label="วันที่เริ่มงาน" type="date" value={formData.start_date} onChange={(v:any) => setFormData({...formData, start_date: v})} disabled={modalMode === 'view'} />
                                    <InputField label="วันที่สิ้นสุด" type="date" value={formData.end_date} onChange={(v:any) => setFormData({...formData, end_date: v})} disabled={modalMode === 'view'} />
                                </FormSection>

                                <FormSection title="ใบอนุญาตขับขี่">
                                    <SelectField label="ประเภทใบขับขี่" value={formData.licence_type} onChange={(v:any) => setFormData({...formData, licence_type: v})} options={options?.licenseTypes} valueKey="license_type_id" labelKey="license_type_name" disabled={modalMode === 'view'} />
                                    <InputField label="เลขที่ใบขับขี่" value={formData.licence_no} onChange={(v:any) => setFormData({...formData, licence_no: v})} disabled={modalMode === 'view'} />
                                    <SelectField label="ออกให้โดย (จังหวัด)" value={formData.licence_by} onChange={(v:any) => setFormData({...formData, licence_by: v})} options={options?.provinces} valueKey="province_id" labelKey="province_name" disabled={modalMode === 'view'} />
                                </FormSection>
                            </div>
                        </div>
...
function InputField({ label, type = "text", value, onChange, placeholder, disabled, required }: any) {
    return (
