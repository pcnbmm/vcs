...
export async function getOrdersForReturn() {
  try {
    ...
    const orgMap = new Map(
      orgs.map((o: any) => [o.orgid.toString(), o.orgname]),
    );

    const ordersWithApprover = orders.map((order) => {
      const approver = order.approve_id
        ? userMap.get(order.approve_id.trim())
        : null;
      const orgName = order.use_div_code
        ? orgMap.get(order.use_div_code.trim())
        : null;
      return {
        ...order,
