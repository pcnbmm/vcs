...
export default function FunctionTab() {
    ...
    const currentFunctions = filteredFunctions.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={3} className="py-12 text-center">
                                        ...
                                        <p className="mt-4 text-sm font-medium text-gray-500">กำลังโหลดข้อมูลฟังก์ชัน...</p>
                                    </td>
                                </tr>
                                ...
                                currentFunctions.map((func) => (
                                    <tr key={func.function_id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="py-4 px-6">
                                            <span className="block text-sm font-bold text-gray-900">{func.func_name}</span>
                                        </td>
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                <button 
                                    ...
                                    className={`w-8 h-8 rounded-lg text-sm font-bold flex items-center justify-center transition-all ${
                                        currentPage === page ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'
                                    }`}
                                >
                                    {page}
                                </button>
                        ...
                        <form onSubmit={handleSave} className="p-6">
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-bold text-gray-700">ชื่อฟังก์ชัน (Function Name) <span className="text-rose-500">*</span></label>
                                    <input 
                                        type="text" 
                                        required
