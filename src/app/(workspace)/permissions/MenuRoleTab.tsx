...
export default function MenuRoleTab() {
  ...
  const toggleMapping = (menuId: number, functionId: number | null) => {
    setFormData((prev) => {
      const exists = prev.mappings.some(
        (m) => m.menu_id === menuId && m.function_id === functionId,
      );
      if (exists) {
        return {
          ...prev,
        ...
        return {
          ...
          mappings: [
            ...prev.mappings,
            { menu_id: menuId, function_id: functionId },
          ],
        };
      }
    });
  ...
  const handleDelete = async (rolesId: number) => {
    if (
      !(await showConfirm(
        "ยืนยันการลบสิทธิ์เมนูและฟังก์ชันทั้งหมดของบทบาทนี้ ใช่หรือไม่?",
      ))
    )
      return;
    try {
      const res = await fetch(`/api/permissions/menu-roles/${rolesId}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-md shadow-sm border border-gray-100">
        ...
        <button
          onClick={() => openModal()}
          className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-rose-600 text-white rounded-lg font-bold text-sm hover:bg-rose-700 shadow-lg shadow-rose-200 transition-all"
        >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider w-[65%]">
                  สิทธิ์เมนูและฟังก์ชันที่ได้รับ (Menus & Functions)
                </th>
                <th className="py-4 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right w-[15%]">
                  จัดการ
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex flex-col gap-3">
                        {mr.menus.map((m: any) => (
                          <div
                            key={m.menu_id}
                            className="bg-gray-50 border border-gray-100 p-3 rounded-lg w-fit min-w-[300px]"
                          >
                            ...
                            <div className="flex flex-wrap gap-1.5 pl-6">
                              ...
                              {m.functions.map((f: any) => (
                                <span
                                  key={f.function_id}
                                  className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-white text-gray-600 border border-gray-300 shadow-sm flex items-center gap-1"
                                >
                    ...
                    <td className="py-4 px-6 text-right align-top">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleDelete(mr.roles_id)}
                          className="p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-md transition-colors shrink-0"
                        >
            ...
            <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-white shrink-0">
              <div>
                ...
                <p className="text-sm text-gray-500 font-medium mt-1 ml-4 text-left">
                  เลือกบทบาท 1 รายการ
                  และติ๊กเลือกเมนู/ฟังก์ชันที่ต้องการให้เข้าถึงสิทธิ์
                </p>
              </div>
              <button
                onClick={closeModal}
            ...
            >
              ...
              <div className="flex justify-end gap-3 pt-4 sticky bottom-0 bg-gray-50/90 backdrop-blur-sm -mx-8 px-8 -mb-8 pb-8 z-10">
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-6 py-3 rounded-md font-bold text-sm text-gray-500 hover:bg-gray-200 bg-gray-100 transition-colors shadow-sm"
                >
                  ยกเลิก
                </button>
                <button
                  ...
                  className="flex items-center gap-2 px-8 py-3 bg-rose-600 text-white rounded-md font-bold text-sm hover:bg-rose-700 shadow-md shadow-rose-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSaving ? (
