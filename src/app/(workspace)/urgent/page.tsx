...
export default function UrgentRequestPage() {
    ...
    const [requesters, setRequesters] = useState<{ userid: number; firstname: string | null; lastname: string | null; departmentid: string | null }[]>([]);
        ...
        const fetchUsers = async () => {
            ...
            if (result.success && result.data) {
                ...
                if (result.data.length > 0) {
                    setFormData(prev => ({ ...prev, requester_id: result.data[0].userid.toString() }));
                }
            } else {
    ...
    const handleSave = async () => {
        if (!formData.requester_id || !formData.destination || !formData.startDate || !formData.startTime || !formData.objective) {
            alert('กรุณากรอกข้อมูลที่จำเป็นให้ครบถ้วน (ผู้ขอใช้งาน, จุดหมาย, วันที่/เวลาเริ่ม, วัตถุประสงค์)');
            return;
        }
    ...
    const resetForm = () => {
        setFormData({
            requester_id: requesters.length > 0 ? requesters[0].userid.toString() : '',
            ownerDept: 'ฝ่ายบริหาร',
            vehicleType: 'รถเก๋ง 1500 cc',
            origin: 'หลักสี่',
            province: 'กรุงเทพมหานคร',
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            ...
            {/* Header Section */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] shadow-sm border border-red-100">
                <div className="flex items-center gap-5">
                    <div className="w-16 h-16 bg-red-600 rounded-2xl flex items-center justify-center shadow-lg shadow-red-200">
                        <FileText className="w-8 h-8 text-white" />
                    </div>
                    <div>
                        <h1 className="text-3xl font-extrabold text-red-600 tracking-tight">คำขอใช้งานด่วน</h1>
                    ...
                    <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-red-100 relative overflow-hidden">
                        ...
                        <div className="relative space-y-10">
                            <div className="flex items-center gap-3 border-b border-red-200 pb-6">
                                ...
                                <h2 className="text-2xl font-black text-gray-900 uppercase tracking-tight">รายละเอียดแผนการเดินทาง</h2>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
                                {/* Requester (ผู้ขอใช้งาน) - Database Driven */}
                                <div className="md:col-span-2">
                                    <FormField label="ผู้ขอใช้งาน (Data from DB)" icon={User} required>
                                        <select
                                            ...
                                            {isLoadingUsers ? (
                                                ...
                                                requesters.map((r) => (
                                                    <option key={r.userid} value={r.userid}>
                                                        {r.firstname} {r.lastname} {r.departmentid ? `(${r.departmentid})` : ''}
                                                    </option>
                                                ))
                                            )}
                                        </select>
                                ...
                                <div className="md:col-span-2">
                                    <label className="flex items-center gap-3 p-4 rounded-2xl hover:bg-red-50 transition-colors cursor-pointer border border-transparent hover:border-red-200">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-bold text-red-900">ขอขับเอง (Self Drive)</span>
                                        </div>
                                    </label>
                                </div>
