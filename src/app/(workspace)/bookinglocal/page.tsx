...
import { getMyLocalBookingSection } from "@/app/actions/orgActions";
...
export default function VehicleRequestPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [sectionRes, driverRes] = await Promise.all([
        getMyLocalBookingSection(),
        getDrivers(),
      ]);

      // Access guard: ต้องมี sectionid ใน vc_own_div_prop เท่านั้น
      if (!sectionRes.authorized || !sectionRes.org) {
        setNotAuthorized(true);
        setAccessChecked(true);
        return;
      }

      setMySection(sectionRes.org);
      setFormData((prev) => ({
        ...prev,
        ownerDept: String(sectionRes.org!.orgid),
      }));
  ...
  // แสดงหน้า forbidden ถ้าไม่มีสิทธิ์
  if (notAuthorized) {
    return (
      <div className="flex items-center justify-center min-h-[40vh]">
        <div className="bg-white rounded-2xl shadow-sm border border-red-100 p-12 flex flex-col items-center gap-5 max-w-md w-full">
          <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center">
            <X className="w-8 h-8 text-red-500" />
          </div>
          <h2 className="text-xl font-bold text-black">ไม่มีสิทธิ์เข้าถึง</h2>
          <p className="text-sm text-gray-500 text-center">
            หน้านี้สำหรับผู้ใช้งานที่มีสังกัดส่วนภูมิภาคที่ได้รับอนุญาตเท่านั้น
            กรุณาติดต่อผู้ดูแลระบบเพื่อขอสิทธิ์
          </p>
          <button
            onClick={() => router.push("/booking")}
            className="px-8 py-3 bg-blue-600 text-white rounded-lg font-bold text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-200"
          >
            ไปยังหน้าขอใช้รถส่วนกลาง
          </button>
        </div>
      </div>
    );
            ...
            <div className="relative space-y-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
                ...
                <FormField label="สังกัดเจ้าของรถ" icon={Users} required>
                  <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 rounded-2xl px-5 py-3.5">
                ...
                <FormField label="สถานที่ (ต้นทาง)" icon={NavIcon} required>
                  <input
                    ...
                    placeholder="ระบุสถานที่ต้นทาง เช่น ส่วนขายและบริการลูกค้า ขอนแก่น"
                    className="w-full bg-gray-50 border-gray-300 border rounded-lg px-4 py-3.5 text-sm focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all font-bold text-black shadow-sm placeholder:text-gray-400 placeholder:font-normal"
                  />
                </FormField>
