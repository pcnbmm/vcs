...
export async function GET() {
  try {
    ...
    const licenseTypes = await prisma.vc_driver_license_type.findMany({ where: notDeleted });
    ...
    // New options
    const provinces = await prisma.vc_province.findMany({ where: notDeleted, orderBy: { province_name: 'asc' } });
    ...
    const typeRegis = await prisma.vc_type_regis.findMany({ where: notDeleted });
