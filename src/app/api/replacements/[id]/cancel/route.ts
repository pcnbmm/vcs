...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!carId || !originalCarNumber) {
            return NextResponse.json({ error: "Missing original car mapping data" }, { status: 400 });
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await tx.vc_car_master.update({
                ...
                data: {
                    ...
                    ...(normalStatus ? { car_status_id: normalStatus.car_status_id } : {}),
        ...
        return NextResponse.json({ error: "Failed to cancel replacement", details: error.message }, { status: 500 });
    }
}
