    ...
    try {
        ...
        if (status_id === 2) {
            ...
            if (requestData) {
                ...
                const uid = requestData.userid ?? requestData.UserID ?? requestData.userId;
                const dest = requestData.journey_place ?? requestData.Journey_place ?? requestData.journeyPlace;
                const causes = requestData.journey_causes ?? requestData.Journey_causes ?? requestData.journeyCauses;
                const jDate = requestData.journey_date ?? requestData.Journey_date ?? requestData.journeyDate;
                ...
                if (uid) {
                    ...
                    if (user && user.email) {
                        await sendApproveEmail({
                            to: user.email,
                            requesterName: user.firstname ? `${user.firstname} ${user.lastname || ''}` : String(uid),
                        ...
                        console.log(`[EmailService] ไม่สามารถส่งอีเมลได้ เนื่องจากไม่พบ email ของผู้ขอใช้รถ (User ID: ${uid})`);
                    }
                }
            }
        }
